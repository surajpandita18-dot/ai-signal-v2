...
export interface IssuePayload {
  ...
  // Multi-agent pipeline telemetry (analyst's chosen structure + correction
  // loop stats). See .claude/architecture-multi-agent-pipeline.md. Optional
  // — issues generated by the old single-synth path won't have it.
  pipeline_metadata?: PipelineMetadata | null
  // Editorial framework v2 fields (added 2026-06-17). All optional — old
  // issues render without them; new issues populate via synthesizer prompt.
  // See CLAUDE.md "Editorial discipline".
  signal_of_the_week?: string | null            // 20-30 words, the ONE screenshot-worthy line
  explained_simply?: {                          // Feynman/Ng-style breakdown of ONE concept
    concept: string                             // ≤6 words
    explanation: string                         // 60-100 words, physical analogy first
  } | null
  production_questions?: string[] | null        // 3 real builder questions this week (not interview prep)
}

// Multi-agent pipeline telemetry. Lives on IssuePayload as an opt-in
// surface; the side table `issue_agent_runs` carries the per-stage detail.
...
export interface PipelineMetadata {
  ...
  spine_beats: Beat[]
  analyst_rationale: string         // one sentence
  rounds_run: number                // 0..2 typically; 0 means first pass passed
  unresolved_flags: number          // chief edits still open after final round
  forward_rate: number              // persona panel: 0..1
}
